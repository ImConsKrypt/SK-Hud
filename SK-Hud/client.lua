...
local function getVehicleHealth(vehicle)
    local updateTick = GetGameTimer()
    if (updateTick - lastHealthUpdate) > 2000 then
...
CreateThread(function()
    ...
    while true do        
        ...
        if cache.vehicle and not IsThisModelABicycle(cache.vehicle) then
            ...
            local vehHealth = getVehicleHealth(cache.vehicle)
            vehicleStats.health = vehHealth.Body
            vehicleStats.engHealth = vehHealth.Engine
            DisplayRadar(true)
        else
